...
__all__ = ["MFBaseUSV", "eighk","MFBase"]
...
class MFBaseUSV():    
    """  
    Base of this is F=(A^2)^(1/2) 
    MFBaseUSV(data, show_progress=False)
...
class MFBase():
    """
    Base of this is F=(A^2)^(1/2) 
    MF Base Class. Poviding some basic methods.
    ...
    def frobenius_norm(self):
        ...
        if hasattr(self,'H') and hasattr(self,'W'):
            if scipy.sparse.issparse(self.data):
                ...
                err = self.data[:,:] - np.dot(self.W, self.H)
                err = np.sqrt(np.sum(err**2))           
        else:
            err = None
    ...
    def _update_h(self):
        """ Overwrite for updating H. if need
        """
        pass

    def _update_w(self):
        """ Overwrite for updating W. if need
        """
        pass
