...
class TestSVD():
    ...
    assert_almost_equal(np.abs(mdl.U), np.abs(U), decimal=2)
    assert_almost_equal(np.abs(mdl.S), np.abs(S), decimal=2)
    assert_almost_equal(np.abs(mdl.V), np.abs(V), decimal=2)
